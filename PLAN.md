...
- [x] Retrieved own examples: `examples_for()`, idf-weighted lemma cosine ≥ 0.3 against every saved aligned pair, top 3, before the paragraph context in the user message; `OWN_EXAMPLES=0` kills it; the pick record carries `examples`. Golden set: kept (Baselines).
...
- 2026-09-17 Sprint 3 on deepseek-v4-pro vs `checks`. Own examples (`s3-examples`): 14 of 100 sentences had a saved neighbour at cosine ≥ 0.3 (the store is one narrator, 270 pairs; hits are the recurring phrases — господа → "dear readers", уверяю вас, наврал/злость). Overall A/B/C 45.1/51.9/41.6 (+0.3/+2.5/+0.1, CIs straddle 50 %); on the 14 items with examples B +8.1 (8/13 wins), C +3.5 (9/13), A −1.3 (1/5). Kept, default on: the effect is where the mechanism says it should be, and there is nothing on the other 86. Floor 0.3 chosen from the score distribution: one-directional coverage let any long sentence win, cosine does not. Free text (`s3-free`, JSON off): 44.5/50.5/42.4 (−0.4/+1.2/+0.9), 0 bad, 0 retries, nothing malformed. Within noise, so JSON stays.
